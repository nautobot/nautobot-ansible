...
| parent | raw | 3.0.0 | With state `new`, it will force to get the next available IP in
this prefix.
...
| state | str |  | Use `present`, `new` or `absent` for adding, force adding or removing.
...
example).
 |
| status | raw | 3.0.0 | The status of the IP address Required if _state=present_ and does not exist yet |

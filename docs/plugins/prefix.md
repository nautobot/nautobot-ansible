...
| prefix_length | int | 3.0.0 | Required ONLY if state is `present` and first_available is `yes`.
...
| status | raw | 3.0.0 | The status of the prefix Required if _state=present_ and does not exist yet |

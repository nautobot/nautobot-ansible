...
def test_query_api_query_error_dictionary(nautobot_api_base):
    with pytest.raises(AnsibleError) as exc:
        test_class = NautobotGraphQL(
            query={"ntc": "networktocode"}, api=nautobot_api_base, variables=None
        )
...
def test_query_api_query_variables_wrong_type(
    nautobot_api_base, graphql_test_query_with_var
):
    with pytest.raises(AnsibleError) as exc:
        test_class = NautobotGraphQL(
            query=graphql_test_query_with_var, api=nautobot_api_base, variables=["ntc"]
        )

"""Tests for Nautobot GraphQL Info Action Plugin."""

import pytest
from ansible.errors import AnsibleError

try:
    from plugins.action.graphql_info import nautobot_action_graphql_info
except ImportError:
    import sys
    ...
    from graphql_info import nautobot_action_graphql_info
...
def test_setup_api_error_missing_url(nautobot_valid_args):
    ...
    with pytest.raises(AnsibleError) as exc:
        test_class = nautobot_action_graphql_info(args)
...
def test_setup_api_error_incorrect_validate_certs(nautobot_valid_args):
    ...
    with pytest.raises(AnsibleError) as exc:
        test_class = nautobot_action_graphql_info(args=nautobot_valid_args)
...
def test_query_api_query_variables_wrong_type(nautobot_valid_args, graphql_test_query_with_var):
    ...
    with pytest.raises(AnsibleError) as exc:
        test_result = nautobot_action_graphql_info(args=nautobot_valid_args)

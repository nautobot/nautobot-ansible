...
DATABASES = {
    "default": {
        "NAME": os.getenv(
            "NAUTOBOT_DATABASE", "{{ nautobot_db_name }}"
        ),  # Database name
        "USER": os.getenv(
            "NAUTOBOT_USER", "{{ nautobot_db_username }}"
        ),  # Database username
        "PASSWORD": os.getenv(
            "NAUTOBOT_PASSWORD", "{{ nautobot_db_password }}"
        ),  # Datbase password
...
CACHEOPS_REDIS = "redis://:{{ redis_password }}@localhost:6379/1"

...
options:
  controller_managed_device_group:
    description:
      - Controller managed device group to assign the radio profile to
    required: false
    type: raw
  radio_profile:
    description:
      - Radio profile to assign to the controller managed device group
    required: false
    type: raw
"""

EXAMPLES = r"""
- name: Create a controller managed device group radio profile assignment
  networktocode.nautobot.controller_managed_device_group_radio_profile_assignment:
    ...
    radio_profile: "My Radio Profile"
    state: present

- name: Delete a controller managed device group radio profile assignment
  networktocode.nautobot.controller_managed_device_group_radio_profile_assignment:
    url: http://nautobot.local
    token: thisIsMyToken
    controller_managed_device_group: "My Device Group"
...
- name: Delete a controller managed device group radio profile assignment by id
  networktocode.nautobot.controller_managed_device_group_radio_profile_assignment:
    url: http://nautobot.local
    token: thisIsMyToken
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            radio_profile=dict(required=False, type="raw"),
        )
    )
